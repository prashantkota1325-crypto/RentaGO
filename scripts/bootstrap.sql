...
ACCEPT RENTAGO_DB_PASSWORD CHAR HIDE PROMPT 'RentaGO database password: '
...
BEGIN
  ...
  IF v_cnt = 0 THEN
    EXECUTE IMMEDIATE 'CREATE USER RENTAGO IDENTIFIED BY "' || '&RENTAGO_DB_PASSWORD' || '" DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS';
    EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE TO RENTAGO';
    DBMS_OUTPUT.PUT_LINE('Created user RENTAGO');
  ELSE
    EXECUTE IMMEDIATE 'ALTER USER RENTAGO IDENTIFIED BY "' || '&RENTAGO_DB_PASSWORD' || '" DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USERS';
